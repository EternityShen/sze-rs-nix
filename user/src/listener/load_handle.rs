use std::{path::PathBuf, sync::{Arc, Mutex, RwLock, mpsc::Sender}};
...
pub fn load_sender(send: Sender<LoadLevel>, sleep_time: Arc<Mutex<u64>>, on_or_off: Arc<Mutex<bool>>, scheduler_config: Arc<RwLock<SchedulerConfig>>) {
...
impl EbpfLoadHandle {
    pub fn new(log_handle: Arc<Mutex<LogHandle>>) -> Self {  
        ...
        let mut bpf = Ebpf::load_file("./system/lib/libebpf.so").context("加载eBPF对象失败").unwrap();
        if let Ok(mut log) = log_handle.lock() {
            log.clear();
            log.info(format!("加载eBPF对象成功"));
        }
        let program: &mut TracePoint = bpf.program_mut("sched_switch").context("获取sched_switch程序失败").unwrap().try_into().unwrap();
        if let Ok(mut log) = log_handle.lock() {
        ...
        program.attach("sched", "sched_switch").context("挂载sched_switch程序失败").unwrap();
    ...
    pub fn read_load(&mut self,) -> u32 {
        ...
        let cpu_stat_map: Array<_, CpuStat> = Array::try_from(self.bpf.map_mut("CPU_STATS").unwrap()).unwrap();
        
        for cpu in 0..self.ncpu {
            let v = cpu_stat_map.get(&cpu, 0).unwrap();
            let delta_busy_ns = v.busy_ns.saturating_sub(self.prev_snapshot[cpu as usize].busy_ns);
            let delta_idle_ns = v.idle_ns.saturating_sub(self.prev_snapshot[cpu as usize].idle_ns);
...
pub fn ebpf_load_sender(send: Sender<LoadLevel>, sleep_time: Arc<Mutex<u64>>, on_or_off: Arc<Mutex<bool>>, scheduler_config: Arc<RwLock<SchedulerConfig>>) {
    let mut load_handle = EbpfLoadHandle::new(Arc::new(Mutex::new(LogHandle::new("./log/ebpf.log"))));
